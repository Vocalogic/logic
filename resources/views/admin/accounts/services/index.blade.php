<div class="row">
    ...
    @if($account->items()->count() == 0)
        ...
        <div class="card">
            <div class="card-body text-center p-5">
                ...
                <a href="#recurringModal" data-bs-toggle="modal"  class="btn btn-{{bm()}}primary border lift">Add Service</a>
            </div>
        </div>
    ...
    <div class="col-lg-9">
        ...
        <a class="btn btn-{{bm()}}primary wait" href="/admin/accounts/{{$account->id}}/statement"><i
                class="fa fa-download"></i> Download Statement</a>

            <a class="btn btn-{{bm()}}primary live" data-title="Schedule Service Suspension" href="/admin/accounts/{{$account->id}}/suspend"><i
                    class="fa fa-clock-o"></i> Schedule Service Suspension</a>

            <a class="btn btn-{{bm()}}primary live" data-title="Schedule Service Termination" href="/admin/accounts/{{$account->id}}/terminate"><i
        ...
        <table class="table table-striped table-sm mt-3">
                        ...
                        <tr>
                            <td><a data-title="Edit {{$item->item->name}}" class="live"
                                   ...
                                   data-title="{{$item->item->name}}">
                                    <strong>[{{$item->item->code}}] {{$item->item->name}}</strong></a>  @if($item->item->addons()->count())
                                    <a data-bs-toggle='tooltip' class="live" data-title="Manage Service Addons" title='Manage Service Addons'
                                       href="/admin/accounts/{{$account->id}}/items/{{$item->id}}/addons"><i class="fa fa-database"></i></a>
                                @endif
                                ...
                                @if($item->quote)
                                    <span class="badge bg-{{bm()}}info">via quote #{{$item->quote->id}}</span>
                                @endif
                            ...
                            @if($item->addons()->count())
                                    ...
                                    @foreach($item->addons as $addon)
                                        <small class="text-muted">&nbsp;&nbsp; - <a class="confirm"
                                                                                    ...
                                                                                    data-method="DELETE"
                                                                                    href="/admin/accounts/{{$account->id}}/items/{{$item->id}}/addons/{{$addon->id}}"><i class="fa fa-trash"></i></a>
                                            <strong>{{$addon->option->addon->name}}</strong> : {{$addon->name}} x {{$addon->qty}} (${{moneyFormat($addon->qty * $addon->price)}})</small> <br/>
                                    @endforeach
                                @endif
                            </td>
            ...
            @if($account->commissionable && $account->partner)
                <tr>
                    <td colspan="4" style="text-align:right">
                        <strong class="text-secondary">Commission to {{$account->partner->name}}: </strong>${{moneyFormat($account->commissionable)}}
                    </td>
                </tr>
                <tr>
                    <td colspan="4" style="text-align:right">
                        <strong class="text-primary">NET Monthly: </strong>${{moneyFormat($account->mrr - $account->commissionable)}}
                    </td>
                </tr>
        ...
        <a href="#recurringModal" data-bs-toggle="modal" data-backdrop="false" class="btn btn-{{bm()}}primary"><i
...
<div class="modal fade" id="recurringModal" tabindex="-2" role="dialog" aria-labelledby="liveModalLabel"
     aria-hidden="true">
    <div class="modal-dialog modal-dialog-vertical modal-dialog-scrollable">
        <div class="modal-content">

            <div class="px-xl-4 modal-header">
                <h5 class="modal-title">Add Monthly Service</h5>
