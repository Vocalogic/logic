<table class="table table-sm table-striped">
    ...
    @foreach($order->items()->where('product', 1)->get() as $item)
        <tr>
            ...
            <td>
                @if($item->shipment)
                    ...
                    <a href="/admin/orders/{{$order->id}}/items/{{$item->id}}/shipment"
                       data-title="Assign or Create new Shipment" class="liveRight"><i class="fa fa-edit"></i></a>
                @else
                    <a href="/admin/orders/{{$order->id}}/items/{{$item->id}}/shipment"
                       data-title="Assign or Create new Shipment" class="liveRight"><i class="fa fa-plus"></i></a>
                @endif
            ...
            <td><a href="/admin/orders/{{$order->id}}/items/{{$item->id}}/notes" class="liveRight"
